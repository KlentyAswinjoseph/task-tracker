...
router.get("/", async (req, res) => {
  try {
    const squads = await Squad.find().sort({ createdAt: -1 }).lean();
...
router.put("/:id", async (req, res) => {
  try {
    ...
    squad.description =
      description !== undefined ? description : squad.description;
...
router.post("/:id/members", async (req, res) => {
  try {
    ...
    const newMembers = memberIds
      .filter(
        (userId) => !squad.members.some((member) => member.userId === userId)
      )
      .map((userId) => {
...
router.delete("/:id/members/:userId", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to remove member from squad" });
  }
});
...
// Get all squads for filtering
router.get('/filter/list', async (req, res) => {
  try {
    ...
    console.error('Error fetching squads for filter:', error);
    return res.status(500).json({ error: 'Failed to fetch squads for filter' });
  }
});
